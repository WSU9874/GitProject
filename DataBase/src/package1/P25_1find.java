...
public class P25_1find {
	...
	public static void main(String[] args) throws SQLException, ClassNotFoundException, UnsupportedEncodingException {
		...
		double k20_lat = 37.3860521; //현위치 위도, 경도
		double k20_lng = 127.1214038;

		String k20_QueryTxt; //문자열 선언
//		k20_QueryTxt = String.format(
//				"select * from cctv where " + "SQRT( POWER( latitude - %f,2) + POWER ( longitude - %f,2)) = "
//						+ "(select MIN( SQRT( POWER( latitude - %f, 2) + POWER(longitude - %f,2))) from cctv);",
//				k20_lat, k20_lng, k20_lat, k20_lng);
//		//가까운 위치 찾기 / 현위치와 테이블에서 위도 경로를 select한 값을 뺀후 가장 적은 것을 구한다.

//		k20_QueryTxt = "select *from cctv";
//		//전체 select
		k20_QueryTxt = "select *from cctv  where manage_office = '경상남도 김해시청'";
		//service_provider가 경상남도 김해시청인 것만 select
//		k20_QueryTxt = "select *from cctv where purpose = '방범용'";
//		//inst_country가 방범용인 것만 select
